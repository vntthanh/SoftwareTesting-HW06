...
- Collection SHA-256: `F679AADE649F0AD84A3873E83A2DD4152107DEA619194C579F7FDBC9DFFD4F20`
- Deterministic whole-collection static validation: **PASS** (2026-08-22 00:01:30 GMT+7). Verified 82 logical IDs, 88 generated requests, all six expected two-request flows, no unexpected/missing IDs, no undefined variables, and no request body/header mismatches against the reviewed CSV representation. A separate validation path independently checked all 36 structured `Headers`/`Body`/`Raw Body` rows without using the generator's `request_specs()` mapping.
- Full supplied Postman Collection v2.1 schema validation: **PASS** (2026-08-22 00:01:38 GMT+7) using `postman/postman-v2.1.0-schema.json`.
- Newman 6.2.2 structural/script compatibility: **PASS** (2026-08-22 00:01:49 GMT+7). One representative API-001 request ran against a local mock; collection-level header injection, fixture gating, request serialization, console scripting, and the status assertion executed successfully.
